...
export type FrontierManifestImpactReasonKind =
  | 'query'
  | 'id'
  | 'file'
  | 'asset'
...
interface ManifestIndex {
  entryIds: string[];
  taskIds: string[];
  entriesById: Map<string, FrontierManifestEntry>;
  tasksById: Map<string, FrontierManifestTask>;
  entryIdsByKind: Map<string, string[]>;
  entryIdsByFeature: Map<string, string[]>;
  taskIdsByFeature: Map<string, string[]>;
  entryIdsByPackage: Map<string, string[]>;
  taskIdsByPackage: Map<string, string[]>;
  entryIdsByOwner: Map<string, string[]>;
  taskIdsByOwner: Map<string, string[]>;
  entryIdsByFile: Map<string, string[]>;
  entryIdsByAsset: Map<string, string[]>;
  taskIdsByInput: Map<string, string[]>;
  taskIdsByOutput: Map<string, string[]>;
  entryIdsByRoute: Map<string, string[]>;
  entryIdsByAction: Map<string, string[]>;
  entryIdsByState: Map<string, string[]>;
  entryIdsByMigration: Map<string, string[]>;
  entryIdsByTest: Map<string, string[]>;
  entryIdsByResource: Map<string, string[]>;
  entryIdsByTag: Map<string, string[]>;
  taskIdsByTag: Map<string, string[]>;
  dependentsById: Map<string, string[]>;
}

const MANIFEST_INDEX = new WeakMap<FrontierManifest, ManifestIndex>();
const GLOB_REGEX_CACHE = new Map<string, RegExp>();

export function createManifest(input: FrontierManifestInput = {}): FrontierManifest {
...
export function parseCodeowners(text: string): FrontierManifestOwnerRule[] {
  const rules: FrontierManifestOwnerRule[] = [];
  ...
  for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
    ...
    for (let i = 1; i < columns.length; i++) {
      if (columns[i].startsWith('#')) break;
      owners[owners.length] = columns[i];
    }
    if (pattern.length === 0 || owners.length === 0) continue;
...
export function validateManifest(manifest: FrontierManifest): FrontierManifestDiagnostic[] {
  return validateManifestParts(manifest.entries, manifest.tasks, manifest.ownerRules);
}

export function queryManifest(manifest: FrontierManifest, input: FrontierManifestQueryInput): FrontierManifestQueryResult {
  ...
  const entries = selectedEntryIds
    ...
    .filter((entry): entry is FrontierManifestEntry => entry !== undefined && (text === undefined || entryMatchesText(entry, text)));
  const tasks = selectedTaskIds
    .map((id) => index.tasksById.get(id))
    .filter((task): task is FrontierManifestTask => task !== undefined && (text === undefined || taskMatchesText(task, text)));
  ...
  for (const entry of limitedEntries) collectSet(owners, entry.owners);
  for (const task of limitedTasks) collectSet(owners, task.owners);
...
export function manifestImpact(manifest: FrontierManifest, input: FrontierManifestImpactInput): FrontierManifestImpact {
  const index = getManifestIndex(manifest);
  ...
  const reasons: FrontierManifestImpactReason[] = [];

  for (const entry of result?.entries ?? []) markAffected(selected, reasons, entry.id, 'query');
  for (const task of result?.tasks ?? []) markAffected(selected, reasons, task.id, 'query');
  for (const id of input.ids ?? []) markAffected(selected, reasons, String(id), 'id');

  for (const file of input.changedFiles ?? []) {
    const normalized = normalizeFilePath(file);
    for (const entry of manifest.entries) {
      if (entryMatchesChangedFile(entry, normalized)) markAffected(selected, reasons, entry.id, 'file', normalized);
    }
    for (const task of manifest.tasks) {
      if (taskMatchesChangedFile(task, normalized)) markAffected(selected, reasons, task.id, 'task-input', normalized);
    }
  }
  for (const asset of input.changedAssets ?? []) {
    const normalized = normalizeFilePath(asset);
    for (const id of index.entryIdsByAsset.get(normalized) ?? []) markAffected(selected, reasons, id, 'asset', normalized);
    for (const id of index.taskIdsByOutput.get(normalized) ?? []) markAffected(selected, reasons, id, 'asset', normalized);
    for (const task of manifest.tasks) {
      if (matchesManifestPatterns(normalized, task.outputs)) markAffected(selected, reasons, task.id, 'asset', normalized);
    }
  }
  for (const resource of input.changedResources ?? []) {
    const normalized = normalizeManifestResource(resource);
    for (const id of index.entryIdsByResource.get(normalized) ?? []) markAffected(selected, reasons, id, 'resource', normalized);
  }

  const ownerRuleIds: string[] = [];
  for (const file of input.changedFiles ?? []) {
    const normalized = normalizeFilePath(file);
    ...
    if (ownerRule !== undefined) {
      ...
      for (const owner of ownerRule.owners) {
        for (const id of index.entryIdsByOwner.get(owner) ?? []) markAffected(selected, reasons, id, 'owner-rule', ownerRule.id);
        for (const id of index.taskIdsByOwner.get(owner) ?? []) markAffected(selected, reasons, id, 'owner-rule', ownerRule.id);
      }
    }
  }
  ...
  const entries = entryIds.map((id) => index.entriesById.get(id)).filter((entry): entry is FrontierManifestEntry => entry !== undefined);
  const tasks = taskIds.map((id) => index.tasksById.get(id)).filter((task): task is FrontierManifestTask => task !== undefined);
...
function createImpactQueryInput(input: FrontierManifestImpactInput): FrontierManifestQueryInput | undefined {
  const query: FrontierManifestQueryInput = {
...
function hasImpactQueryFields(input: FrontierManifestQueryInput): boolean {
...
export function createManifestFeatureMap(manifest: FrontierManifest): FrontierManifestFeatureMap {
  const index = getManifestIndex(manifest);
  const features = new Set<string>();
  for (const key of index.entryIdsByFeature.keys()) features.add(key);
  for (const key of index.taskIdsByFeature.keys()) features.add(key);
  const nodes: FrontierManifestFeatureNode[] = [];
...
export function createManifestRegistryGraph(
  manifest: FrontierManifest,
  input: Omit<FrontierRegistryGraph, 'kind' | 'version' | 'entries' | 'records' | 'edges'> = {}
  ...
  for (const entry of manifest.entries) {
    ...
    for (const owner of entry.owners) edges[edges.length] = { from: entryNode(entry.id), to: ownerNode(owner), kind: 'owned-by' };
    for (const file of entry.files) edges[edges.length] = { from: entryNode(entry.id), to: fileNode(file), kind: 'declared-in' };
    for (const route of entry.routes) edges[edges.length] = { from: entryNode(entry.id), to: resourceNode('route:' + route), kind: 'handles' };
    for (const asset of entry.assets) edges[edges.length] = { from: entryNode(entry.id), to: fileNode(asset), kind: 'touches' };
  }
  for (const task of manifest.tasks) {
    entries[entries.length] = taskToRegistryEntry(task);
    for (const owner of task.owners) edges[edges.length] = { from: entryNode(task.id), to: ownerNode(owner), kind: 'owned-by' };
    for (const inputPattern of task.inputs) edges[edges.length] = { from: entryNode(task.id), to: patternNode(inputPattern), kind: 'consumes' };
    for (const outputPattern of task.outputs) edges[edges.length] = { from: entryNode(task.id), to: patternNode(outputPattern), kind: 'produces' };
  }
  for (const rule of manifest.ownerRules) {
    ...
    for (const owner of rule.owners) edges[edges.length] = { from: entryNode(rule.id), to: ownerNode(owner), kind: 'owns' };
  }
...
export function createManifestRecord(entry: FrontierManifestEntry): FrontierRegistryRecord {
  return {
...
export function mergeManifests(manifests: readonly FrontierManifest[], input: Omit<FrontierManifestInput, 'entries' | 'tasks' | 'ownerRules' | 'codeowners'> = {}): FrontierManifest {
  const entries: FrontierManifestEntryInput[] = [];
  const tasks: FrontierManifestTaskInput[] = [];
  const ownerRules: FrontierManifestOwnerRuleInput[] = [];
  for (const manifest of manifests) {
    for (const entry of manifest.entries) entries[entries.length] = entry;
    for (const task of manifest.tasks) tasks[tasks.length] = task;
    for (const rule of manifest.ownerRules) ownerRules[ownerRules.length] = rule;
  }
...
export function encodeManifestJsonl(manifest: FrontierManifest): string {
  const lines = [
    stableStringify({
  ...
  for (const entry of manifest.entries) lines[lines.length] = stableStringify({ type: 'entry', entry });
  for (const task of manifest.tasks) lines[lines.length] = stableStringify({ type: 'task', task });
  for (const ownerRule of manifest.ownerRules) lines[lines.length] = stableStringify({ type: 'ownerRule', ownerRule });
  return lines.join('\n') + '\n';
}

export function decodeManifestJsonl(text: string): FrontierManifest {
  const entries: FrontierManifestEntryInput[] = [];
  const tasks: FrontierManifestTaskInput[] = [];
  const ownerRules: FrontierManifestOwnerRuleInput[] = [];
  ...
  for (const raw of String(text).split(/\r?\n/)) {
    ...
    if (parsed.kind === FRONTIER_MANIFEST_JSONL_KIND) {
      ...
      entries[entries.length] = parsed.entry as FrontierManifestEntryInput;
    } else if (parsed.type === 'task') {
      tasks[tasks.length] = parsed.task as FrontierManifestTaskInput;
    } else if (parsed.type === 'ownerRule') {
      ownerRules[ownerRules.length] = parsed.ownerRule as FrontierManifestOwnerRuleInput;
    }
  }
  return createManifest({ entries, tasks, ownerRules, generatedAt, root, metadata });
}

export function createManifestProof(manifest: FrontierManifest): FrontierManifestProof {
  const payload = {
    kind: manifest.kind,
...
export function matchesManifestPattern(file: string, pattern: string): boolean {
  return globToRegExp(pattern).test(normalizeFilePath(file));
}

export function matchesManifestPatterns(file: string, patterns: readonly string[]): boolean {
  const normalized = normalizeFilePath(file);
  let matched = false;
  for (const rawPattern of patterns) {
    const pattern = normalizeFilePattern(rawPattern);
    if (pattern.startsWith('!')) {
      if (pattern.length > 1 && matchesManifestPattern(normalized, pattern.slice(1))) matched = false;
...
function normalizeEntry(input: FrontierManifestEntryInput): FrontierManifestEntry {
...
function normalizeOwnerRule(input: FrontierManifestOwnerRuleInput): FrontierManifestOwnerRule {
...
function normalizeTask(input: FrontierManifestTaskInput): FrontierManifestTask {
...
function applyOwnerRules(entries: FrontierManifestEntry[], tasks: FrontierManifestTask[], rules: readonly FrontierManifestOwnerRule[]): void {
...
function ownersForFiles(files: readonly string[], rules: readonly FrontierManifestOwnerRule[]): string[] {
  const owners: string[] = [];
  for (const file of files) {
    const rule = ownerRuleForFile(file, rules);
    if (rule !== undefined) for (const owner of rule.owners) addUnique(owners, owner);
  }
  return owners.sort();
}

function ownerRuleForFile(file: string, rules: readonly FrontierManifestOwnerRule[]): FrontierManifestOwnerRule | undefined {
  let matched: FrontierManifestOwnerRule | undefined;
  for (const rule of rules) {
    if (matchesManifestPattern(file, rule.pattern)) matched = rule;
  }
  return matched;
}

function validateManifestParts(
  entries: readonly FrontierManifestEntry[],
  tasks: readonly FrontierManifestTask[],
  ownerRules: readonly FrontierManifestOwnerRule[]
): FrontierManifestDiagnostic[] {
  const diagnostics: FrontierManifestDiagnostic[] = [];
  const knownIds = new Set<string>();
  for (const entry of entries) knownIds.add(entry.id);
  for (const task of tasks) knownIds.add(task.id);
  const ids = new Set<string>();
  for (const entry of entries) {
    if (ids.has(entry.id)) diagnostics[diagnostics.length] = error('duplicate-id', 'manifest id is declared more than once: ' + entry.id, { entryId: entry.id });
    ids.add(entry.id);
  ...
  for (const task of tasks) {
    if (ids.has(task.id)) diagnostics[diagnostics.length] = error('duplicate-id', 'manifest id is declared more than once: ' + task.id, { taskId: task.id });
    ids.add(task.id);
  ...
  for (const rule of ownerRules) {
    if (ruleIds.has(rule.id)) diagnostics[diagnostics.length] = error('duplicate-owner-rule', 'owner rule id is declared more than once: ' + rule.id, { ownerRuleId: rule.id });
    ruleIds.add(rule.id);
    if (rule.owners.length === 0) diagnostics[diagnostics.length] = warning('owner-rule-empty', 'owner rule has no owners: ' + rule.id, { ownerRuleId: rule.id });
  }
  return diagnostics;
}

function summarizeManifestParts(
  entries: readonly FrontierManifestEntry[],
  tasks: readonly FrontierManifestTask[],
  ownerRules: readonly FrontierManifestOwnerRule[],
  diagnostics: readonly FrontierManifestDiagnostic[]
): FrontierManifestSummary {
  const features = new Set<string>();
  const owners = new Set<string>();
  ...
  for (const task of tasks) {
    if (task.feature !== undefined) features.add(task.feature);
    if (task.package !== undefined) packages.add(task.package);
    collectSet(owners, task.owners);
    collectSet(files, task.inputs);
    collectSet(assets, task.outputs);
    collectSet(tags, task.tags);
  }
  for (const rule of ownerRules) collectSet(owners, rule.owners);
  for (const diagnostic of diagnostics) if (diagnostic.severity === 'error') errorCount++;
...
function getManifestIndex(manifest: FrontierManifest): ManifestIndex {
...
function createManifestIndex(manifest: FrontierManifest): ManifestIndex {
  ...
  for (const entry of manifest.entries) {
    ...
    if (entry.feature !== undefined) appendMapArray(index.entryIdsByFeature, entry.feature, entry.id);
    if (entry.package !== undefined) appendMapArray(index.entryIdsByPackage, entry.package, entry.id);
    ...
    for (const dependency of entry.dependsOn) appendMapArray(index.dependentsById, dependency, entry.id);
  }
  for (const task of manifest.tasks) {
    index.taskIds[index.taskIds.length] = task.id;
    index.tasksById.set(task.id, task);
    if (task.feature !== undefined) appendMapArray(index.taskIdsByFeature, task.feature, task.id);
    if (task.package !== undefined) appendMapArray(index.taskIdsByPackage, task.package, task.id);
    ...
    for (const dependency of task.dependsOn) appendMapArray(index.dependentsById, dependency, task.id);
  }
  return index;
}

function manifestEntryToRegistryEntry(entry: FrontierManifestEntry): FrontierRegistryEntry {
  return {
    id: entry.id,
    kind: entry.kind,
...
function taskToRegistryEntry(task: FrontierManifestTask): FrontierRegistryEntry {
  return {
    id: task.id,
    kind: 'task',
...
function markAffected(
  ...
  if (selected.has(id) && kind !== 'dependency') return;
  selected.add(id);
...
function propagateManifestDependencies(index: ManifestIndex, selected: Set<string>, reasons: FrontierManifestImpactReason[]): void {
...
function createImpactSeeds(input: FrontierManifestImpactInput, result: FrontierManifestQueryResult | undefined): string[] {
...
function entryMatchesChangedFile(entry: FrontierManifestEntry, file: string): boolean {
  if (entry.files.includes(file) || entry.assets.includes(file)) return true;
  for (const pattern of entry.files.concat(entry.assets)) if (isPattern(pattern) && matchesManifestPattern(file, pattern)) return true;
  return false;
}

function taskMatchesChangedFile(task: FrontierManifestTask, file: string): boolean {
  return matchesManifestPatterns(file, task.inputs);
}

function entryMatchesText(entry: FrontierManifestEntry, text: string): boolean {
  return [
    entry.id,
    entry.kind,
    entry.name,
  ...
  ].filter((value): value is string => value !== undefined).join(' ').toLowerCase().includes(text);
}

function taskMatchesText(task: FrontierManifestTask, text: string): boolean {
  return [
    task.id,
    task.command,
    task.package,
...
function collectOwners(entries: readonly FrontierManifestEntry[], tasks: readonly FrontierManifestTask[]): string[] {
  const values = new Set<string>();
  for (const entry of entries) collectSet(values, entry.owners);
  for (const task of tasks) collectSet(values, task.owners);
  return Array.from(values).sort();
}

function collectFiles(entries: readonly FrontierManifestEntry[]): string[] {
  const values = new Set<string>();
  for (const entry of entries) collectSet(values, entry.files);
  return Array.from(values).sort();
}

function collectAssets(entries: readonly FrontierManifestEntry[], tasks: readonly FrontierManifestTask[]): string[] {
  const values = new Set<string>();
  for (const entry of entries) collectSet(values, entry.assets);
  for (const task of tasks) collectSet(values, task.outputs);
  return Array.from(values).sort();
}

function collectEntryValues<K extends 'routes' | 'actions' | 'states' | 'migrations' | 'tests' | 'resources'>(
  entries: readonly FrontierManifestEntry[],
  key: K
): string[] {
  const values = new Set<string>();
  for (const entry of entries) collectSet(values, entry[key]);
  return Array.from(values).sort();
}

function collectFeatures(entries: readonly FrontierManifestEntry[], tasks: readonly FrontierManifestTask[]): string[] {
  const values = new Set<string>();
  for (const entry of entries) if (entry.feature !== undefined) values.add(entry.feature);
  for (const task of tasks) if (task.feature !== undefined) values.add(task.feature);
  return Array.from(values).sort();
}

function collectPackages(entries: readonly FrontierManifestEntry[], tasks: readonly FrontierManifestTask[]): string[] {
  const values = new Set<string>();
  for (const entry of entries) if (entry.package !== undefined) values.add(entry.package);
  for (const task of tasks) if (task.package !== undefined) values.add(task.package);
  return Array.from(values).sort();
}

function collectTags(entries: readonly FrontierManifestEntry[], tasks: readonly FrontierManifestTask[]): string[] {
  const values = new Set<string>();
  for (const entry of entries) collectSet(values, entry.tags);
  for (const task of tasks) collectSet(values, task.tags);
  return Array.from(values).sort();
}

function cloneQuery(input: FrontierManifestQueryInput): FrontierManifestQueryInput {
  return {
...
function cloneEntry(entry: FrontierManifestEntry): FrontierManifestEntry {
  return {
    ...entry,
...
function cloneTask(task: FrontierManifestTask): FrontierManifestTask {
  return {
    ...task,
...
function cloneOwnerRule(rule: FrontierManifestOwnerRule): FrontierManifestOwnerRule {
  return {
    ...rule,
...
function idsFromMap(map: Map<string, string[]>, values: readonly string[] | undefined): string[] | undefined {
  if (values === undefined) return undefined;
  const out: string[] = [];
  for (const value of values) {
    const ids = map.get(value);
    if (ids !== undefined) for (const id of ids) addUnique(out, id);
  }
  return out;
}

function taskIdsForFiles(
  index: ManifestIndex,
  tasks: readonly FrontierManifestTask[],
  ...
  for (const file of files) {
    for (const id of exactMap.get(file) ?? []) addUnique(out, id);
    for (const task of tasks) {
      const patterns = direction === 'input' ? task.inputs : task.outputs;
      if (matchesManifestPatterns(file, patterns)) addUnique(out, task.id);
    }
  }
  return out;
}

function intersectCandidate(current: string[] | null, next: readonly string[] | undefined): string[] | null {
...
function appendMapValues(map: Map<string, string[]>, values: readonly string[], id: string): void {
  for (const value of values) appendMapArray(map, value, id);
}

function appendMapArray(map: Map<string, string[]>, key: string, value: string): void {
  const bucket = map.get(key);
  if (bucket === undefined) map.set(key, [value]);
  else if (!bucket.includes(value)) bucket[bucket.length] = value;
}

function mapToRecord(...maps: Array<Map<string, string[]> | undefined>): Record<string, string[]> {
  const out: Record<string, string[]> = {};
  for (const map of maps) {
    if (map === undefined) continue;
    for (const [key, values] of map) {
      const bucket = out[key] ?? [];
      for (const value of values) addUnique(bucket, value);
      out[key] = bucket.sort();
    }
  }
...
function normalizeFilePattern(value: string): string {
...
function normalizeFileList(values: readonly string[] | undefined): string[] {
  return uniqueStrings((values ?? []).map(normalizeFilePath)).sort();
}

function normalizePathList(values: readonly FrontierRegistryPath[] | undefined): string[] {
...
function cloneSource(source: FrontierRegistrySource | undefined): FrontierRegistrySource | undefined {
  if (source === undefined) return undefined;
  if (Array.isArray(source)) return source.map((item) => ({ ...item, file: normalizeFilePath(item.file) }));
  const location = source as { file: string };
  return { ...source, file: normalizeFilePath(location.file) };
}

function sourceFiles(source: FrontierRegistrySource | undefined): string[] {
...
function hasAnyArray(values: readonly string[], set: Set<string>): boolean {
...
function compareReason(left: FrontierManifestImpactReason, right: FrontierManifestImpactReason): number {
  return left.id.localeCompare(right.id) || left.kind.localeCompare(right.kind) || (left.via ?? '').localeCompare(right.via ?? '');
}

function collectSet(target: Set<string>, values: readonly string[]): void {
...
function addUnique(out: string[], value: string): void {
  if (value.length !== 0 && !out.includes(value)) out[out.length] = value;
}
...
function ownerNode(owner: string): string {
  return 'owner:' + owner;
}
...
function patternNode(pattern: string): string {
  return 'pattern:' + pattern;
}

function resourceNode(resource: string): string {
  return normalizeManifestResource(resource);
}

function error(code: string, message: string, extra: Partial<FrontierManifestDiagnostic>): FrontierManifestDiagnostic {
  return { severity: 'error', code, message, ...extra };
}

function warning(code: string, message: string, extra: Partial<FrontierManifestDiagnostic>): FrontierManifestDiagnostic {
...
function cloneJsonObject(value: unknown): JsonObject | undefined {
  ...
  return json !== undefined && json !== null && typeof json === 'object' && !Array.isArray(json) ? json as JsonObject : undefined;
}
...
function sortJson(value: JsonValue | undefined): JsonValue | undefined {
  if (value === undefined || value === null || typeof value !== 'object') return value;
  if (Array.isArray(value)) return value.map(sortJson) as JsonValue[];
  const out: JsonObject = {};
  for (const key of Object.keys(value).sort()) out[key] = sortJson((value as JsonObject)[key]) as JsonValue;
  return out;
}
